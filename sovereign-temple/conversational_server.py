...
class ConversationalSovereign:
    ...
    async def generate_response(self, session_id: str, user_message: str) -> str:
        ...
        self.conversations[session_id].append({
            "role": "user", 
            "content": user_message + care_context
        })
        
        # Keep only last 20 messages for context
        if len(self.conversations[session_id]) > 21:
            self.conversations[session_id] = [self.conversations[session_id][0]] + self.conversations[session_id][-20:]
        ...
        try:
            async for message in websocket:
                ...
                if data.get("type") == "text" or data.get("type") == "speech":
                    ...
                    # Generate response
                    response_text = await self.sovereign.generate_response(session_id, user_text)
