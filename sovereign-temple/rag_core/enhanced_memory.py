...
class EnhancedMemoryStore:
    ...
    async def consolidate_memories_by_tag(self, tag: str, window_hours: int = 24) -> Dict[str, Any]:
        ...
        if len(rows) <= 50:
            return {"consolidated": 0, "message": f"Only {len(rows)} memories with tag '{tag}' in window — no consolidation needed"}
    ...
    async def query_memories(self,
        ...
        for mem in memories:
            ...
            if care_wt >= care_weight_min:
                ...
                scored_memories.append((care_boost + importance_boost, len(scored_memories), mem))
